...
import { PATH_API_PRESIGNED_URL } from '@/config/site-paths';
import {
  CURRENT_STORAGE,
  HAS_AWS_S3_STORAGE,
...
const moveFile = async (originUrl: string, destinationFileName: string) => {
  ...
  switch (storageType) {
    case 'vercel-blob':
      url = await vercelBlobCopy(originUrl, destinationFileName, false);
      break;
    case 'cloudflare-r2':
      ...
      break;
    case 'aws-s3':
      url = await awsS3Copy(originUrl, destinationFileName, false);
      break;
  }
...
export const convertUploadToPhoto = async (urlOrigin: string) => {
