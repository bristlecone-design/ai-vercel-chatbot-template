import { TAG_FAVS, getValidationMessageForTags } from '@/features/photo/tag';
import {
  FILM_SIMULATION_FORM_INPUT_OPTIONS,
  MAKE_FUJIFILM,
} from '@/features/photo/vendors/fujifilm';
...
import { getAspectRatioFromExif, getOffsetFromExif } from '@/lib/exif';
import { nanoid } from '@/lib/id';
import { roundToNumber } from '@/lib/number';
import { convertStringToArray } from '@/lib/strings';

import { DEFAULT_ASPECT_RATIO, type PhotoExif } from '..';

import { GEO_PRIVACY_ENABLED } from '@/config/site-settings';
import type {
  AnnotatedTag,
  FormMeta,
  Photo,
...
export const convertFormDataToPhotoDbInsert = (
  ...
  return {
    ...
    focalLength: photoForm.focalLength
      ? Number.parseInt(photoForm.focalLength)
      : undefined,
    ...
    fNumber: photoForm.fNumber
      ? Number.parseFloat(photoForm.fNumber)
      : undefined,
    latitude: photoForm.latitude
      ? Number.parseFloat(photoForm.latitude)
      : undefined,
    longitude: photoForm.longitude
      ? Number.parseFloat(photoForm.longitude)
      : undefined,
    iso: photoForm.iso ? Number.parseInt(photoForm.iso) : undefined,
    exposureTime: photoForm.exposureTime
      ? Number.parseFloat(photoForm.exposureTime)
      : undefined,
    ...
    priorityOrder: photoForm.priorityOrder
      ? Number.parseFloat(photoForm.priorityOrder)
      : undefined,
