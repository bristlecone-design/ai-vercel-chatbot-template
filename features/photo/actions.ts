...
import { isTagFavs, TAG_FAVS } from '@/tag';
import { createStreamableValue } from 'ai/rsc';

import { runAuthenticatedAdminServerAction } from '@/lib/auth/edge-auth';
import { streamOpenAiImageQuery } from '@/lib/openai/openai-images';
import { convertUploadToPhoto, deleteStorageUrl } from '@/lib/storage';
...
import { convertFormDataToPhotoDbInsert, convertPhotoToFormData } from './form';
    ...
    (async () => {
      try {
        for (const [index, { url }] of uploadUrls.entries()) {
          ...
          const { photoFormExif, imageResizedBase64 } =
        ...
        stream.error(
          `${error.message} (${addedUploadUrls.length} of ${uploadTotal} photos successfully added)`
        );
      } finally {
        revalidateAllKeysAndPaths();
      }
...
export const queryPhotosByTitleAction = async (query: string) =>
  (await getPhotos({ query, limit: 10 })).filter(({ title }) => Boolean(title));
