import { generateOpenAiImageQuery } from '@/lib/openai/openai-images';
  ...
  if (imageBase64) {
    if (
      ...
      if (textFieldsToGenerate.includes('title')) {
        title = cleanUpAiTextResponse(
          await generateOpenAiImageQuery(imageBase64, AI_IMAGE_QUERIES['title'])
        );
      }
    ...
    if (textFieldsToGenerate.includes('tags')) {
      tags = cleanUpAiTextResponse(
        await generateOpenAiImageQuery(imageBase64, AI_IMAGE_QUERIES['tags'])
      );
    }
