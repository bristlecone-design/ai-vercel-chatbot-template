...
import { type Cameras, createCameraKey } from '@/features/camera';
import {
  type PhotoDateRange,
  parsePhotoFromDb,
  translatePhotoId,
} from '@/features/photo';
import type { TagsWithMeta } from '@/features/photo/tag';
import type { FilmSimulation, FilmSimulations } from '@/features/simulation';

import { query, sql } from '@/lib/postgres';
import { convertArrayToPostgresString } from '@/lib/postgres/postgres-utils';
...
import { SHOULD_DEBUG_SQL } from '@/config/site-settings';
import type { PhotoDbInsert, Photo, PhotoDb } from '@/types/photo';
...
export const renamePhotoTagGlobally = async (tag: string, updatedTag: string) =>
  safelyQueryPhotos(
    () => sql`
    UPDATE photos
...
export const getPhotosNearId = async (
  ...
  safelyQueryPhotos(async () => {
    ...
    return query(
      ...
      const indexNumber = photo ? Number.parseInt(photo.row_number) : undefined;
      return {
...
export const getPhotosMeta = async (options: GetPhotosOptions = {}) =>
  safelyQueryPhotos(async () => {
