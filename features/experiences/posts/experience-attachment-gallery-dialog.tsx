...
export interface ExperienceAttachmentGalleryDialogProps extends DialogProps {
  noCloseBtn?: boolean;
...
export function ExperienceAttachmentGalleryDialog({
  open,
  children,
  className,
  mediaClassName,
  ...
  title: titleProp = 'Media Gallery',
  content: contentProp,
  collaborator,
  handleOnClose,
  ...props
}: ExperienceAttachmentGalleryDialogProps) {
   ...
   *
   * We only have 3 images, but we paginate them absolutely (ie 1, 2, 3, 4, 5...) and then wrap that within 0-2 to find our image ID in the array below. By passing an absolute page index as the `motion` component's `key` prop, `AnimatePresence` will detect it as an entirely new image. So you can infinitely paginate as few as 1 images.
   */
  const mediaIndex = wrap(0, mediaCount, page);
  const mediaItem = attachments[mediaIndex];
  const { url, name, contentType } = mediaItem || {};
  ...
  // Geo
  const mediaHasGeo = false; // Boolean(latitude && longitude);
  ...
  useKey(
    ...
    { event: 'keyup' },
    [page, hasMultipleMedia, showGeoMap, mediaCount]
  );
  ...
  // Content
  const attachmentContent = contentProp;
  const title = titleProp;
  const finalTitle = title;
  const userName = collaborator?.name || collaborator?.username || '';
  const userAvatar =
    collaborator?.avatar ||
    collaborator?.image ||
    collaborator?.avatar ||
  ...
  // Media Type
  const mediaExtension = getFileExtension(url);
  const mediaIsImage = mediaExtension
    ? isImageExtension(mediaExtension)
    : isImage(url);

  const mediaIsVideo = mediaExtension
    ? isVideoExtension(mediaExtension)
    : isVideo(url);
  ...
  return (
    <Drawer
      key={'experience-media-attachment-gallery'}
      {...props}
      modal
      // handleOnly
      open={isOpen}
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          dragConstraints={hasMultipleMedia ? { left: 0, right: 0 } : undefined}
          dragElastic={hasMultipleMedia ? 1 : undefined}
          onDragEnd={
            hasMultipleMedia
              ? (e, { offset, velocity }) => {
